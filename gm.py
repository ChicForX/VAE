...
class VAE(nn.Module):
    ...
    def kl_divergence(self, res):
        ...
        cross_entropy = 3.0 * self.entropy_loss(res['prob'])
    ...
    def entropy_loss(self, prob):
        negative_log_prob = prob * torch.log(prob + 1e-20)  # Adding a small value to avoid log(0)
        entropy = -torch.sum(torch.sum(negative_log_prob, dim=-1))
        return entropy
